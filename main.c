...
int main() {
    ...
    if(strcmp("login",check_if_login_or_register(user_input)) == 0) {
        ...
        while(fgets(file_line, 255, Banking_Information)){
            ...
            All_Accounts[total_accounts] = Add_Account(All_Accounts[total_accounts],file_line);

            if(strcmp(strlwr(Eliminate_first_word(user_input)), strlwr(Take_Name_Surname(file_line))) == 0){
                Same_Owner_accounts[nr_accounts_of_person] = Add_Account(Same_Owner_accounts[nr_accounts_of_person],file_line);
    ...
    //fprintf(New_Banking_Information, "%s\n", header);

    printf("\n\n");
    for(int i = 0; i < total_accounts; i++) {
        printf("%s" ,All_Accounts[i]);
        free(All_Accounts[i]);
    }
    ...
    //fprintf(New_Banking_Information, "I know math");
